...
namespace SelfServiceAPI.Controllers
{
    ...
    public class SourceController : ApiController
    {
        ...
        public List<getSourceList_Result> GetSources()
        {
            ...
            {
                return entities.getSourceList(applicationFormSettingId).ToList();
            }
        }

        [HttpPost]
        public HttpResponseMessage Sources(List<SourceRequet> lstRequest)
        {
            ...
            try
            {
                if (lstRequest != null && lstRequest.Count > 0)
                {
                    using (ApplicationFormEntities entities = new ApplicationFormEntities())
                    {
                        foreach (SourceRequet request in lstRequest)
                        {
